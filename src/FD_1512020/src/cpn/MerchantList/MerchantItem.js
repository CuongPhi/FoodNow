...
export default class MerchantItem extends Component {
    ...
    render() {
        const { textTag, main, image, infoLayout, distanceLayout, textName, textAddr, tagLayout } = styles
        const {item} = this
        return (
            <View style={main}>
                <Image
                ...
                <View style={infoLayout}>
                    <Text style={textName} ellipsizeMode='tail' numberOfLines={1}>{item.name}</Text>
                    <Text style={textAddr} ellipsizeMode='tail' numberOfLines={1}>{item.address}</Text>
